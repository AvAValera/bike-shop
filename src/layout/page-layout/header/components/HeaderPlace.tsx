...
export default function HeaderPlace() {
  const { setLocation } = settings((state) => state);
  ...
  const localStore = window.localStorage.getItem("settings");
  ...
  const [city, setCity] = useState<string>(() => {
    ...
    else return locationCitys[0].name;
  });
      ...
      {showLocation && (
        <Menu>
          <Tooltip label="location" openDelay={1000}>
            <MenuButton
              className="hover-header-btn flex-non"
              p={2}
              outlineOffset={"none"}
            >
              <Flex gap={1}>
                <img src="/layout/header/marker.svg" alt="" />
                <span className="text-base text-black@ whitespace-nowrap hidden lg:inline">
                  {city}
                </span>
              </Flex>
            </MenuButton>
